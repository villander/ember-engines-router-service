...
class EngineRouterService extends Service.extend(Evented) {
  ...
  refresh(routeName = this.currentRouteName, ...args) {
    if (macroCondition(dependencySatisfies('ember-source', '>= 4.1.0'))) {
      ...
      return this.externalRouter.refresh(namespaceEngineRouteName(this._mountPoint, routeName), ...args);
    } else {
  ...
  transitionTo(routeName, ...args) {
    ...
    return this.externalRouter.transitionTo(namespaceEngineRouteName(this._mountPoint, routeName), ...args);
  }
  ...
  replaceWith(routeName, ...args) {
    ...
    return this.externalRouter.replaceWith(namespaceEngineRouteName(this._mountPoint, routeName), ...args);
  }
